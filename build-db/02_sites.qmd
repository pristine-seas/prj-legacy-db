...
core_site_fields <- c("exp_id", "leg", "survey_type", "ps_site_id", 
...
core_site_schema <- tibble::tibble(
  ...
  Type = c("STRING", "STRING", "STRING", "STRING", 
           ...
           "FLOAT", "FLOAT", 
           "STRING", "STRING"),
  Required = c(rep(TRUE, 12), FALSE),
  Description = c(
    "Unique expedition identifier in the format `ISO3_YYYY` (e.g., PNG_2024).",
    "Cruise leg or operational phase (e.g., Leg 1, Pacific, North Coast).",
    "Survey type. Allowed values: uvs, sbruvs, pbruvs, sub, rov, dscm, bird, ysi, edna.",
    "Unique Pristine Seas site ID in the format `ISO3_YYYY_survey_###` (e.g., PNG_2024_uvs_001).",
    "Broad region or administrative unit (e.g., Murat, Chocó, Tuamotu, Duff Islands).",
    "Intermediate subregion such as an island, atoll, gulf, or reef complex (e.g., Mussau, Golfo de Tribugá).",
    "Local named feature such as a community, bay, cove, reef, or village (e.g., Lolieng, Ensenada Utría).",
    ...
    "Name of the individual who led the fieldwork at this site.",
    "Free-text notes describing the site or any irregularities.")
)
...
Pristine Seas uses a standardized three-tier geographic hierarchy to describe the spatial context of each sampling site. These fields are used consistently across all methods and expeditions, enabling robust filtering, mapping, and spatial analysis.

```{r geo_hirarchy, eval = T, include = T}
#| label: tbl-geo-hierarchy
#| tbl-cap: "Geographic Hierarchy Schema"


tibble::tibble(
  Field = c("`region`", "`subregion`", "`locality`"),
  Description = c("A broad administrative, ecological, or biogeographic unit. Typically aligns with a province, marine ecoregion, or large island group (e.g., *Murat*, *Manus*, *Chocó*, *Temotu*).",
                  "A mid-scale feature within the region. Often refers to a single island, atoll, gulf, reef complex, or archipelago (e.g., *Mussau*, *Bipi*, *Golfo de Tribugá*, *Nendo*).",
                  "A specific place within the subregion — such as a bay, reef, village, point, or cove (e.g., *Lolieng*, *6 Islands Reef*, *Ensenada de Utría*, *Lata*).")) |>
  gt() |>
  cols_label(
    Field = "Field",
...
Additional fields include a `site_name` (often used for repeat surveys), protection status, and flags indicating which UVS sub-methods were conducted at each site (@tbl-uvs-site-schema).
...
uvs_site_fields <- tibble::tibble(Method = "uvs",
                                  Field = c("site_name", "habitat", "exposure", 
                                            "protected",  "blt", "lpi", "ysi", "inverts", "recruits", "edna", "photomosaic"),
                                  Type = c(rep("STRING", 3),
                                           rep("BOOLEAN", 8)),
